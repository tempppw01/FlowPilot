...
test('auto-run controller continues later rounds after no phone supply even when auto retry is disabled', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      if (events.runCalls === 1) {
        throw new Error('SMSBower no numbers available across 1 country candidate(s): United States: NO_NUMBERS; providerIds=3193.');
      }
    },
    runtime,
    setState: async (updates = {}) => {
  ...
  assert.equal(events.runCalls, 2, 'no phone supply should continue to the next round without auto retry');
  assert.equal(events.accountRecords.length, 1, 'the exhausted round should still be recorded as failed');
  assert.match(events.accountRecords[0].reason, /NO_NUMBERS/i);
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, 'a later round is still available');
  assert.ok(events.logs.some(({ message }) => /继续下一轮并重新获取手机号/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});
...
test('auto-run controller continues next batch round after repeated SMSBower fresh-attempt restarts', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      ...
      if (events.runCalls <= 2) {
        throw new Error('RESTART_CURRENT_ATTEMPT::步骤 7：SMSBower TempMail 未检测到可复用登录态，放弃当前账号。');
      }
      currentState = {
        ...currentState,
        stepStatuses: {
  ...
  assert.equal(events.runCalls, 3);
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false);
  assert.equal(currentState.autoRunPhase, 'complete');
  assert.equal(events.logs.some(({ message }) => /继续下一轮/.test(message)), true);
});
