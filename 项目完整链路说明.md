...
- SMSBower 设置提供“随机 / 按顺序 / 固定”三种国家模式按钮：随机模式使用非美国国家池，按顺序模式使用可拖选的国家优先级队列，固定模式只使用填写的 country ID；模式切换会同步保存，并按模式显示对应的国家配置区域
...
- 注册方式 `signupMethod`
- 接码开关、接码平台 `phoneSmsProvider`，以及 HeroSMS / 5sim 各自的 API Key、国家/地区、价格上限和平台扩展设置；SMSBower 额外保存 `smsbowerCountryMode`、`smsbowerCountryOrder`、`smsbowerFixedCountryId` 与 `smsbowerRandomMode`
- iCloud 相关偏好
- LuckMail API 配置
