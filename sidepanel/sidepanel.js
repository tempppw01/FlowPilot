...
const inputSmsBowerMailDomain = document.getElementById('input-smsbower-mail-domain');
const inputSmsBowerMailMaxPrice = document.getElementById('input-smsbower-mail-max-price');
...
const btnSmsBowerApiDocs = document.getElementById('btn-smsbower-api-docs');
...
const btnSmsBowerCountryModeRecommended = document.getElementById('btn-smsbower-country-mode-recommended');
...
const smsbowerFixedCountryMenuShell = document.getElementById('smsbower-fixed-country-menu-shell');
const btnSmsBowerFixedCountryMenu = document.getElementById('btn-smsbower-fixed-country-menu');
const smsbowerFixedCountryMenu = document.getElementById('smsbower-fixed-country-menu');
...
const smsbowerProviderMenuShell = document.getElementById('smsbower-provider-menu-shell');
const btnSmsBowerProviderMenu = document.getElementById('btn-smsbower-provider-menu');
const smsbowerProviderMenu = document.getElementById('smsbower-provider-menu');
...
let smsbowerProviderIdsAutoValue = '3193';
let smsbowerProviderIdsManualOverride = false;
let smsbowerProviderIdsManualValue = '';
let smsbowerProviderIdsManualSelection = false;
...
const SMSBOWER_RECOMMENDED_ROUTE_ITEMS = Object.freeze([
  { countryId: 187, countryLabel: '美国', providerId: '3193', price: 0.064 },
  { countryId: 151, countryLabel: '智利', providerId: '3451', price: 0.052 },
  { countryId: 151, countryLabel: '智利', providerId: '3452', price: 0.052 },
  { countryId: 151, countryLabel: '智利', providerId: '3419', price: 0.052 },
  { countryId: 5560, countryLabel: '坦桑尼亚', providerId: '2268', price: 0.054 },
]);
const SMSBOWER_LOW_PRICE_COUNTRY_ITEMS = Object.freeze([
  ...
  { id: 187, label: '美国', englishLabel: 'USA', price: '0.064', providerIds: '3193' },
]);
const SMSBOWER_COUNTRY_CODE_BY_ID = Object.freeze({
  6: 'ID', 10: 'VN', 16: 'GB', 19: 'NG', 22: 'IN', 31: 'ZA', 33: 'CO', 39: 'AR',
  43: 'DE', 46: 'SE', 48: 'NL', 52: 'TH', 53: 'SA', 54: 'MX', 73: 'BR', 78: 'FR',
  85: 'MD', 95: 'AE', 151: 'CL', 160: 'BZ', 215: 'XK', 269: 'IS', 708: 'BI',
  976: 'AM', 1099: 'TJ', 2058: 'LT', 3118: 'HK', 5560: 'TZ', 18397: 'GR', 187: 'US',
});
...
function getSmsBowerCountryFlagById(id) {
  const countryId = normalizeSmsBowerCountryIdValue(id, 0);
  return countryCodeToFlag(SMSBOWER_COUNTRY_CODE_BY_ID[countryId]);
}
...
const PHONE_SMS_PROVIDER_UI_DESCRIPTORS = Object.freeze({
  ...
  [PHONE_SMS_PROVIDER_SMSBOWER]: Object.freeze({
    supportsReusableActivation: false,
    supportsManualFreeReuse: false,
    supportsFreeReusePreservation: false,
    supportsAutomaticFreeReuse: false,
...
const DEFAULT_SMSBOWER_MAIL_BASE_URL = window.SmsBowerMailUtils?.DEFAULT_SMSBOWER_MAIL_BASE_URL || 'https://smsbower.page/api/mail';
const DEFAULT_SMSBOWER_MAIL_SERVICE_CODE = window.SmsBowerMailUtils?.DEFAULT_SMSBOWER_MAIL_SERVICE_CODE || 'dr';
const DEFAULT_SMSBOWER_MAIL_DOMAIN = window.SmsBowerMailUtils?.DEFAULT_SMSBOWER_MAIL_DOMAIN || 'gmail.com';
const DEFAULT_SMSBOWER_MAIL_MAX_PRICE = window.SmsBowerMailUtils?.DEFAULT_SMSBOWER_MAIL_MAX_PRICE || '0.134';
...
function getCurrentRegistrationEmailUiCopy() {
  ...
  const useSmsBowerMail = typeof isSmsBowerMailProvider === 'function'
    ? isSmsBowerMailProvider()
    : String(selectMailProvider.value || '').trim().toLowerCase() === 'smsbower-mail';
  if (useSmsBowerMail) {
    return {
      buttonLabel: '获取',
      placeholder: '点击获取 SMSBower TempMail 邮箱，或手动粘贴邮箱',
      successVerb: '获取',
...
const MAIL_PROVIDER_LOGIN_CONFIGS = {
  ...
  [SMSBOWER_MAIL_PROVIDER]: {
    label: 'SMSBower TempMail',
    url: 'https://smsbower.app/cn/api?page=mails',
    buttonLabel: '文档',
  },
  '2925': {
    label: '2925 邮箱',
...
function applyYydsMailSettingsState(state = {}) {
  ...
  const normalizeSmsBowerMailBaseUrlSafe = typeof normalizeSmsBowerMailBaseUrl === 'function'
    ...
    : ((value = '') => String(value || '').trim() || 'https://smsbower.page/api/mail');
...
function applySmsBowerMailSettingsState(state = {}) {
  if (inputSmsBowerMailApiKey) {
    inputSmsBowerMailApiKey.value = state?.smsbowerMailApiKey || '';
  }
  if (inputSmsBowerMailBaseUrl) {
    inputSmsBowerMailBaseUrl.value = normalizeSmsBowerMailBaseUrl(state?.smsbowerMailBaseUrl);
  }
  if (inputSmsBowerMailServiceCode) {
    inputSmsBowerMailServiceCode.value = normalizeSmsBowerMailServiceCode(state?.smsbowerMailServiceCode);
  }
  if (inputSmsBowerMailDomain) {
    inputSmsBowerMailDomain.value = normalizeSmsBowerMailDomain(state?.smsbowerMailDomain);
  }
  if (inputSmsBowerMailMaxPrice) {
    inputSmsBowerMailMaxPrice.value = normalizeSmsBowerMailMaxPrice(state?.smsbowerMailMaxPrice);
  }
}
...
function collectSettingsPayload() {
  ...
  const normalizeSmsBowerCountryOrderSafe = typeof normalizeSmsBowerCountryOrderValue === 'function'
    ? normalizeSmsBowerCountryOrderValue
    : ((value = []) => (Array.isArray(value) ? value : String(value || '').split(/[\r\n,，;；]+/))
      .map((entry) => Math.floor(Number(entry && typeof entry === 'object' ? (entry.id || entry.countryId || entry.country) : entry)))
      .filter((entry, index, list) => Number.isFinite(entry) && entry > 0 && list.indexOf(entry) === index));
  const normalizeSmsBowerServiceCodeSafe = typeof normalizeSmsBowerServiceCodeValue === 'function'
    ? normalizeSmsBowerServiceCodeValue
    : ((value = '') => String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '') || 'dr');
  const normalizeSmsBowerProviderIdsSafe = typeof normalizeSmsBowerProviderIdsValue === 'function'
    ? normalizeSmsBowerProviderIdsValue
    : ((value = '') => String(value || '').split(/[\s,，;；|]+/).map((entry) => entry.trim()).filter((entry) => /^\d+$/.test(entry)).join(',') || '3170');
  const normalizeSmsBowerPriceSafe = typeof normalizeSmsBowerPriceValue === 'function'
    ? normalizeSmsBowerPriceValue
    : ((value = '') => {
      const numeric = Number(String(value ?? '').trim());
      return Number.isFinite(numeric) && numeric > 0 ? String(Math.round(numeric * 10000) / 10000) : '';
    });
  ...
  const defaultSmsBowerProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
    ? DEFAULT_SMSBOWER_PROVIDER_IDS
    : '3193';
  const smsBowerCountryOrderSelectionValue = typeof smsbowerCountryOrderSelection !== 'undefined' && Array.isArray(smsbowerCountryOrderSelection)
    ? smsbowerCountryOrderSelection
    : [];
  const smsBowerCountryOrderForProviderIds = normalizeSmsBowerCountryOrderSafe(
    smsBowerCountryOrderSelectionValue.length
      ? smsBowerCountryOrderSelectionValue
      : ((Array.isArray(latestState?.smsbowerCountryOrder) && latestState.smsbowerCountryOrder.length) ? latestState.smsbowerCountryOrder : defaultSmsBowerCountryOrder)
  );
  const smsBowerProviderIdsFallbackValue = normalizeSmsBowerProviderIdsSafe(
    typeof getSmsBowerProviderIdsForCountryOrder === 'function'
      ? getSmsBowerProviderIdsForCountryOrder(smsBowerCountryOrderForProviderIds)
      : (() => {
        const providerIds = [];
        const seen = new Set();
        smsBowerCountryOrderForProviderIds.forEach((countryId) => {
          const normalizedCountryId = typeof normalizeSmsBowerCountryIdValue === 'function'
            ? normalizeSmsBowerCountryIdValue(countryId, 0)
            : Math.floor(Number(countryId));
          if (!normalizedCountryId || seen.has(normalizedCountryId)) {
            return;
          }
          seen.add(normalizedCountryId);
          const providerId = normalizedCountryId === 187
            ? '3193'
            : String(normalizedCountryId);
          providerIds.push(providerId);
        });
        return providerIds.join(',') || defaultSmsBowerProviderIds;
      })()
  );
  const smsBowerCountryOrderValue = typeof selectSmsBowerCountryOrder !== 'undefined' && selectSmsBowerCountryOrder
    ? smsBowerCountryOrderForProviderIds
  ...
  const smsBowerProviderIdsValue = typeof inputSmsBowerProviderIds !== 'undefined' && inputSmsBowerProviderIds
    ? normalizeSmsBowerProviderIdsSafe(inputSmsBowerProviderIds.value || smsBowerProviderIdsFallbackValue || latestState?.smsbowerProviderIds)
    : normalizeSmsBowerProviderIdsSafe(latestState?.smsbowerProviderIds || smsBowerProviderIdsFallbackValue || defaultSmsBowerProviderIds);
  const smsBowerProviderIdsManualValue = Boolean(
    (typeof smsbowerProviderIdsManualSelection !== 'undefined' && smsbowerProviderIdsManualSelection)
      || latestState?.smsbowerProviderIdsManual
  );
  const smsBowerMinPriceValue = typeof inputSmsBowerMinPrice !== 'undefined' && inputSmsBowerMinPrice
  ...
  return {
    ...
    smsbowerMailApiKey: (typeof inputSmsBowerMailApiKey !== 'undefined' && inputSmsBowerMailApiKey) ? inputSmsBowerMailApiKey.value.trim() : '',
    smsbowerMailBaseUrl: normalizeSmsBowerMailBaseUrlSafe((typeof inputSmsBowerMailBaseUrl !== 'undefined' && inputSmsBowerMailBaseUrl) ? inputSmsBowerMailBaseUrl.value : ''),
    smsbowerMailServiceCode: normalizeSmsBowerMailServiceCodeSafe((typeof inputSmsBowerMailServiceCode !== 'undefined' && inputSmsBowerMailServiceCode) ? inputSmsBowerMailServiceCode.value : ''),
    smsbowerMailDomain: normalizeSmsBowerMailDomainSafe((typeof inputSmsBowerMailDomain !== 'undefined' && inputSmsBowerMailDomain) ? inputSmsBowerMailDomain.value : ''),
    smsbowerMailMaxPrice: normalizeSmsBowerMailMaxPriceSafe((typeof inputSmsBowerMailMaxPrice !== 'undefined' && inputSmsBowerMailMaxPrice) ? inputSmsBowerMailMaxPrice.value : ''),
    ...
    smsbowerCountryMode: typeof selectSmsBowerCountryMode !== 'undefined' && selectSmsBowerCountryMode?.value === 'fixed'
      ? 'fixed'
      : (typeof selectSmsBowerCountryMode !== 'undefined' && selectSmsBowerCountryMode?.value === 'recommended' ? 'recommended' : 'priority'),
    ...
    smsbowerProviderIdsManual: smsBowerProviderIdsManualValue,
    smsbowerMinPrice: smsBowerMinPriceValue,
    smsbowerMaxPrice: smsBowerMaxPriceValue,
    smsbowerRandomMode: smsBowerRandomModeValue,
...
function normalizePhoneSmsCountryId(value, provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalizedProvider === smsBowerProvider) {
...
function normalizePhoneSmsMaxPriceValue(value = '', provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalizedProvider === smsBowerProvider) {
    return normalizeSmsBowerPriceValue(value);
  }
  return normalizeHeroSmsMaxPriceValue(value);
}
...
function normalizeSmsBowerCountryIdValue(value, fallback = DEFAULT_SMSBOWER_COUNTRY_ORDER[0]) {
  const parsed = Math.floor(Number(value));
  if (Number.isFinite(parsed) && parsed > 0) {
    return parsed;
  }
...
function getSmsBowerCountryLinesById(id) {
  const item = getSmsBowerCountryItemById(id);
  const fallbackPrice = Number(item?.price);
  const fallbackLines = String(item?.providerIds || '')
    .split(/[,\s，；;|]+/)
    .map((providerId) => providerId.trim())
    .filter((providerId) => /^\d+$/.test(providerId))
    .map((providerId) => ({
      providerId,
      price: Number.isFinite(fallbackPrice) ? fallbackPrice : null,
      count: null,
    }));
  const lines = Array.isArray(item?.lines) ? item.lines : fallbackLines;
  return lines
    .map((line) => ({
      providerId: String(line?.providerId ?? line?.id ?? '').trim(),
      price: Number(line?.price),
      count: Number.isFinite(Number(line?.count)) ? Number(line.count) : null,
    }))
    .filter((line) => /^\d+$/.test(line.providerId))
    .sort((left, right) => ((Number.isFinite(left.price) ? left.price : Number.MAX_SAFE_INTEGER)
      - (Number.isFinite(right.price) ? right.price : Number.MAX_SAFE_INTEGER))
      || left.providerId.localeCompare(right.providerId));
}

function formatSmsBowerPriceText(value) {
  const direct = Number(value);
  const price = Number.isFinite(direct) ? direct : Number(String(value ?? '').match(/\d+(?:\.\d+)?/)?.[0]);
  return Number.isFinite(price) && price > 0 ? `$${price.toFixed(4).replace(/0+$/, '').replace(/\.$/, '')}` : '价格--';
}

function formatSmsBowerLineLabel(line = {}, countryId = 0) {
  const countText = Number.isFinite(Number(line.count)) ? ` · 可用 ${Math.max(0, Math.floor(Number(line.count)))}` : '';
  return `线路 ${line.providerId} · ${getSmsBowerCountryFlagById(countryId)} ${getSmsBowerCountryLabelById(countryId)} · ${formatSmsBowerPriceText(line.price)}${countText}`;
}
...
function getSmsBowerDefaultProviderIdsByCountryId(id) {
  const countryId = normalizeSmsBowerCountryIdValue(id, 0);
  if (!countryId) {
    return '';
  }
  const item = getSmsBowerCountryItemById(countryId);
  return normalizeSmsBowerProviderIdsValue(item?.providerIds || item?.providerId || (countryId === 187 ? '3193' : ''));
}

function getSmsBowerProviderIdsForCountryOrder(countryOrder = [], options = {}) {
  const cheapestOnly = options.cheapestOnly === true;
  const normalizedCountryOrder = normalizeSmsBowerCountryOrderValue(countryOrder);
  const providerIds = [];
  const seen = new Set();
  const defaultProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
    ? DEFAULT_SMSBOWER_PROVIDER_IDS
    : '3193';
  normalizedCountryOrder.forEach((countryId) => {
    const providerId = cheapestOnly
      ? getSmsBowerCheapestProviderIdForCountry(countryId)
      : getSmsBowerDefaultProviderIdsByCountryId(countryId);
...
function getSmsBowerCheapestProviderIdForCountry(countryId) {
  const lines = getSmsBowerCountryLinesById(countryId)
    .filter((line) => Number.isFinite(line.price) && line.price > 0);
  const availableLines = lines.filter((line) => line.count === null || line.count > 0);
  const candidates = availableLines.length ? availableLines : lines;
  return candidates[0]?.providerId
    || getSmsBowerDefaultProviderIdsByCountryId(countryId).split(',')[0]
    || '';
}

function syncSmsBowerProviderIdsFromCountrySelection(countryOrder = [], options = {}) {
  const defaultProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
    ? DEFAULT_SMSBOWER_PROVIDER_IDS
    : '3193';
  const nextAutoValue = normalizeSmsBowerProviderIdsValue(
    getSmsBowerProviderIdsForCountryOrder(countryOrder) || defaultProviderIds
  ) || defaultProviderIds;
  const currentValue = normalizeSmsBowerProviderIdsValue(
    inputSmsBowerProviderIds?.value || latestState?.smsbowerProviderIds || ''
  );
  if (typeof smsbowerProviderIdsManualOverride !== 'undefined'
    && smsbowerProviderIdsManualOverride
    && !options.allowManualOverride) {
    const manualValue = normalizeSmsBowerProviderIdsValue(
      smsbowerProviderIdsManualValue || currentValue || nextAutoValue
    );
    if (inputSmsBowerProviderIds) {
      inputSmsBowerProviderIds.value = manualValue;
    }
    return manualValue;
  }
  const shouldUpdate = Boolean(options.force)
    ...
    || currentValue === LEGACY_USA_DEFAULT_SMSBOWER_PROVIDER_IDS;
  if (shouldUpdate && inputSmsBowerProviderIds) {
    inputSmsBowerProviderIds.value = nextAutoValue;
  }
  if (shouldUpdate) {
    smsbowerProviderIdsAutoValue = nextAutoValue;
  }
  return nextAutoValue;
}

function setSmsBowerProviderIdsManualValue(value = '') {
  const normalizedValue = normalizeSmsBowerProviderIdsValue(value);
  smsbowerProviderIdsManualOverride = true;
  smsbowerProviderIdsManualSelection = true;
  smsbowerProviderIdsManualValue = normalizedValue;
  if (inputSmsBowerProviderIds) {
    inputSmsBowerProviderIds.value = normalizedValue;
  }
  return normalizedValue;
}

function preserveSmsBowerProviderIdsManualValue() {
  if (typeof smsbowerProviderIdsManualOverride === 'undefined'
    || !smsbowerProviderIdsManualOverride
    || !inputSmsBowerProviderIds) {
    return false;
  }
  const manualValue = normalizeSmsBowerProviderIdsValue(
    smsbowerProviderIdsManualValue || inputSmsBowerProviderIds.value || ''
  );
  inputSmsBowerProviderIds.value = manualValue;
  smsbowerProviderIdsManualValue = manualValue;
  return true;
}
...
function normalizeSmsBowerProviderIdsValue(value = '') {
  const normalized = String(value || '')
    .split(/[\s,，;；|]+/)
  ...
  const defaultProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
    ? DEFAULT_SMSBOWER_PROVIDER_IDS
    : '3193';
  return normalized || defaultProviderIds;
}

function normalizeSmsBowerPriceValue(value = '') {
  const numeric = Number(String(value ?? '').trim());
  return Number.isFinite(numeric) && numeric > 0 ? String(Math.round(numeric * 10000) / 10000) : '';
}
...
function resolvePhoneSmsPricePreviewRange(provider = '') {
  ...
  const smsBowerProvider = typeof PHONE_SMS_PROVIDER_SMSBOWER !== 'undefined' ? PHONE_SMS_PROVIDER_SMSBOWER : 'smsbower';
  let rawMinPrice = typeof inputHeroSmsMinPrice !== 'undefined' && inputHeroSmsMinPrice ? inputHeroSmsMinPrice.value : '';
  let rawMaxPrice = typeof inputHeroSmsMaxPrice !== 'undefined' && inputHeroSmsMaxPrice ? inputHeroSmsMaxPrice.value : '';
  if (normalizedProvider === PHONE_SMS_PROVIDER_MADAO) {
    rawMinPrice = typeof inputMaDaoMinPrice !== 'undefined' && inputMaDaoMinPrice ? inputMaDaoMinPrice.value : '';
    rawMaxPrice = typeof inputMaDaoMaxPrice !== 'undefined' && inputMaDaoMaxPrice ? inputMaDaoMaxPrice.value : '';
  } else if (normalizedProvider === smsBowerProvider) {
    rawMinPrice = typeof inputSmsBowerMinPrice !== 'undefined' && inputSmsBowerMinPrice ? inputSmsBowerMinPrice.value : '';
    rawMaxPrice = typeof inputSmsBowerMaxPrice !== 'undefined' && inputSmsBowerMaxPrice ? inputSmsBowerMaxPrice.value : DEFAULT_SMSBOWER_MAX_PRICE;
  }
...
function updateHeroSmsPlatformDisplay() {
  ...
  const smsBowerProvider = typeof PHONE_SMS_PROVIDER_SMSBOWER !== 'undefined' ? PHONE_SMS_PROVIDER_SMSBOWER : 'smsbower';
  let selected = getSelectedHeroSmsCountryOption();
  if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
    selected = getSelectedFiveSimCountries()[0] || { id: DEFAULT_FIVE_SIM_COUNTRY_ID, label: DEFAULT_FIVE_SIM_COUNTRY_LABEL };
  } else if (provider === PHONE_SMS_PROVIDER_NEXSMS) {
    selected = getSelectedNexSmsCountries()[0] || { id: DEFAULT_NEX_SMS_COUNTRY_ORDER[0], label: `Country #${DEFAULT_NEX_SMS_COUNTRY_ORDER[0]}` };
  } else if (provider === PHONE_SMS_PROVIDER_MADAO) {
    selected = {
      id: '',
      label: normalizeMaDaoModeValue(selectMaDaoMode?.value || latestState?.madaoMode) === MADAO_MODE_DIRECT
        ? 'direct'
        : getSelectedMaDaoRoutingPlanLabel(),
    };
  } else if (provider === smsBowerProvider) {
    const recommendedMode = selectSmsBowerCountryMode?.value === SMSBOWER_COUNTRY_MODE_RECOMMENDED;
...
function updateSmsBowerCountryOrderSummary(selectedCountries = []) {
  ...
  if (randomModeEnabled) {
    displaySmsBowerCountryOrder.textContent = '随机模式：每次请求随机国家和线路 ID（含美国）';
...
function updateSmsBowerCountryOrderMenuSummary(selectedCountries = []) {
  ...
  const randomCandidateCount = typeof SMSBOWER_LOW_PRICE_COUNTRY_ITEMS !== 'undefined'
    ...
    : normalized.filter((id) => id > 0).length;
...
function setSmsBowerRandomMode(enabled, options = {}) {
  smsbowerRandomModeEnabled = Boolean(enabled);
  if (btnSmsBowerCountryOrderRandom) {
    btnSmsBowerCountryOrderRandom.classList.toggle('is-active', smsbowerRandomModeEnabled);
    btnSmsBowerCountryOrderRandom.setAttribute('aria-pressed', smsbowerRandomModeEnabled ? 'true' : 'false');
    btnSmsBowerCountryOrderRandom.textContent = smsbowerRandomModeEnabled ? '随机模式已开启' : '随机模式';
  }
...
function syncSmsBowerCountrySelectionOrderFromSelect(options = {}) {
  ...
  updateSmsBowerCountryOrderSummary(selectedCountries);
  updateSmsBowerCountryOrderMenuSummary(selectedCountries);
  syncSmsBowerProviderIdsFromCountrySelection(smsbowerCountryOrderSelection, {
    force: Boolean(options.forceProviderIds),
  });
  renderSmsBowerCountryOrderMenu();
...
function randomizeSmsBowerCountryOrder(randomFn = Math.random) {
  ...
  if (typeof showToast === 'function') {
    showToast(`SMSBower 已启用随机国家队列：${randomOrder.length} 个地区（含美国）`, 'ok', 1800);
  }
  return nextOrder;
}
...
async function loadSmsBowerCountries(options = {}) {
  ...
  getSmsBowerCountryItems().forEach((entry) => {
    ...
    option.textContent = entry.price
      ? `${getSmsBowerCountryFlagById(id)} ${entry.label || `Country #${id}`} · ID ${id} · 起价 $${entry.price}`
      : `${getSmsBowerCountryFlagById(id)} ${entry.label || `Country #${id}`} · ID ${id}`;
    selectSmsBowerCountryOrder.appendChild(option);
    smsbowerCountrySearchTextById.set(id, `${entry.label || ''} ${entry.englishLabel || ''} ${entry.providerIds || ''} ${id} ${entry.price} ${entry.countryCode || ''}`.trim());
  });
  ...
  if (typeof renderSmsBowerFixedCountryMenu === 'function') renderSmsBowerFixedCountryMenu();
  if (typeof renderSmsBowerProviderMenu === 'function') renderSmsBowerProviderMenu();
  return getSmsBowerCountryItems();
}

function setSmsBowerFixedCountryMenuOpen(open) {
  const nextOpen = Boolean(open);
  btnSmsBowerFixedCountryMenu?.setAttribute('aria-expanded', String(nextOpen));
  if (smsbowerFixedCountryMenu) smsbowerFixedCountryMenu.hidden = !nextOpen;
}

function updateSmsBowerFixedCountryMenuSummary() {
  const fixedCountryId = Math.max(0, Math.floor(Number(inputSmsBowerFixedCountryId?.value) || 0));
  if (btnSmsBowerFixedCountryMenu) {
    btnSmsBowerFixedCountryMenu.textContent = fixedCountryId
      ? `${getSmsBowerCountryFlagById(fixedCountryId)} ${getSmsBowerCountryLabelById(fixedCountryId)} · ID ${fixedCountryId} · 起价 ${formatSmsBowerPriceText(getSmsBowerCountryPriceById(fixedCountryId))}`
      : '请选择国家';
  }
}

function renderSmsBowerFixedCountryMenu() {
  if (!smsbowerFixedCountryMenu) return;
  smsbowerFixedCountryMenu.innerHTML = '';
  const searchInput = document.createElement('input');
  searchInput.type = 'search';
  searchInput.className = 'data-input mono hero-sms-country-menu-search-input';
  searchInput.placeholder = '搜索国家 / ID';
  smsbowerFixedCountryMenu.appendChild(searchInput);
  const list = document.createElement('div');
  list.className = 'hero-sms-country-menu-list';
  getSmsBowerCountryItems().forEach((entry) => {
    const countryId = normalizeSmsBowerCountryIdValue(entry?.id, 0);
    if (!countryId) return;
    const item = document.createElement('button');
    item.type = 'button';
    item.className = 'header-dropdown-item hero-sms-country-menu-item';
    item.dataset.searchText = getSmsBowerCountrySearchTextById(countryId);
    item.textContent = `${getSmsBowerCountryFlagById(countryId)} ${entry.label || `Country #${countryId}`} · ID ${countryId} · 起价 ${formatSmsBowerPriceText(entry.price)}`;
    item.addEventListener('click', () => {
      inputSmsBowerFixedCountryId.value = String(countryId);
      updateSmsBowerFixedCountryMenuSummary();
      updateSmsBowerCountryModeUI();
      setSmsBowerFixedCountryMenuOpen(false);
      markSettingsDirty(true);
      saveSettings({ silent: true }).catch(() => { });
    });
    list.appendChild(item);
  });
  smsbowerFixedCountryMenu.appendChild(list);
  searchInput.addEventListener('input', () => {
    const keyword = String(searchInput.value || '').trim().toLowerCase();
    Array.from(list.children).forEach((item) => { item.hidden = keyword && !item.dataset.searchText.toLowerCase().includes(keyword); });
  });
}

function setSmsBowerProviderMenuOpen(open) {
  const nextOpen = Boolean(open);
  btnSmsBowerProviderMenu?.setAttribute('aria-expanded', String(nextOpen));
  if (smsbowerProviderMenu) smsbowerProviderMenu.hidden = !nextOpen;
}

function updateSmsBowerProviderMenuSummary() {
  if (selectSmsBowerCountryMode?.value === SMSBOWER_COUNTRY_MODE_RECOMMENDED) {
    if (btnSmsBowerProviderMenu) {
      btnSmsBowerProviderMenu.textContent = `推荐线路：${SMSBOWER_RECOMMENDED_ROUTE_ITEMS.map((entry) => entry.providerId).join(', ')}`;
    }
    return;
  }
  const ids = normalizeSmsBowerProviderIdsValue(inputSmsBowerProviderIds?.value || '').split(',').filter(Boolean);
  if (btnSmsBowerProviderMenu) {
    btnSmsBowerProviderMenu.textContent = ids.length ? `已选 ${ids.length} 条线路：${ids.join(', ')}` : '自动按国家选择线路';
  }
}

function toggleSmsBowerProviderSelection(currentProviderIds = '', providerId = '', fallbackProviderIds = '') {
  const currentText = String(currentProviderIds || '').trim();
  const selected = new Set(currentText
    ? normalizeSmsBowerProviderIdsValue(currentText).split(',').filter(Boolean)
    : []);
  const normalizedProviderId = normalizeSmsBowerProviderIdsValue(providerId).split(',')[0] || '';
  if (!normalizedProviderId) {
    return Array.from(selected).join(',') || normalizeSmsBowerProviderIdsValue(fallbackProviderIds);
  }
  if (selected.has(normalizedProviderId)) {
    selected.delete(normalizedProviderId);
  } else {
    selected.add(normalizedProviderId);
  }
  return Array.from(selected).join(',') || normalizeSmsBowerProviderIdsValue(fallbackProviderIds);
}

function renderSmsBowerProviderMenu() {
  if (!smsbowerProviderMenu) return;
  smsbowerProviderMenu.innerHTML = '';
  const searchWrap = document.createElement('div');
  searchWrap.className = 'hero-sms-country-menu-search';
  const searchInput = document.createElement('input');
  searchInput.type = 'search';
  searchInput.className = 'data-input mono hero-sms-country-menu-search-input';
  searchInput.placeholder = '搜索线路 / 国家 / 价格';
  searchWrap.appendChild(searchInput);
  ...
  if (recommendedMode) {
    SMSBOWER_RECOMMENDED_ROUTE_ITEMS.forEach((line) => {
      lines.set(line.providerId, { ...line, countryId: line.countryId });
    });
  } else {
    ...
    countries.forEach((countryId) => getSmsBowerCountryLinesById(countryId).forEach((line) => {
      ...
      if (!existing || (Number.isFinite(line.price) && line.price < existing.price)) lines.set(line.providerId, { ...line, countryId });
    }));
  }
  const list = document.createElement('div');
  list.className = 'hero-sms-country-menu-list';
  Array.from(lines.values()).sort((left, right) => (left.price || 999) - (right.price || 999)).forEach((line) => {
    const item = document.createElement('button');
    item.type = 'button';
    item.className = 'header-dropdown-item hero-sms-country-menu-item';
    item.dataset.searchText = `${formatSmsBowerLineLabel(line, line.countryId)} ${line.providerId}`.toLowerCase();
    const label = document.createElement('span');
    label.className = 'hero-sms-country-menu-item-label';
    label.textContent = formatSmsBowerLineLabel(line, line.countryId);
    const badge = document.createElement('span');
    badge.className = 'hero-sms-country-menu-item-badge';
    badge.textContent = selected.has(line.providerId) ? '已选' : '';
    item.append(label, badge);
    item.setAttribute('aria-pressed', selected.has(line.providerId) ? 'true' : 'false');
    item.addEventListener('click', () => {
      const nextValue = toggleSmsBowerProviderSelection(
        Array.from(selected).join(','),
        line.providerId,
        getSmsBowerProviderIdsForCountryOrder(countries)
      );
      setSmsBowerProviderIdsManualValue(nextValue);
      updateSmsBowerProviderMenuSummary();
      renderSmsBowerProviderMenu();
      markSettingsDirty(true);
      saveSettings({ silent: true }).catch(() => { });
    });
    list.appendChild(item);
  });
  smsbowerProviderMenu.appendChild(list);
  searchInput.addEventListener('input', () => {
    const keyword = String(searchInput.value || '').trim().toLowerCase();
...
function getSmsBowerProviderMenuCountryIds() {
  if (selectSmsBowerCountryMode?.value === SMSBOWER_COUNTRY_MODE_RECOMMENDED) {
    return Array.from(new Set(SMSBOWER_RECOMMENDED_ROUTE_ITEMS.map((entry) => entry.countryId)));
  }
...
function setSmsBowerCountryMode(mode = SMSBOWER_COUNTRY_MODE_PRIORITY, options = {}) {
  ...
  if (normalizedMode === 'random') {
    ...
    const nextMode = normalizedMode === SMSBOWER_COUNTRY_MODE_FIXED
      ? SMSBOWER_COUNTRY_MODE_FIXED
      : (normalizedMode === SMSBOWER_COUNTRY_MODE_RECOMMENDED
        ? SMSBOWER_COUNTRY_MODE_RECOMMENDED
        : SMSBOWER_COUNTRY_MODE_PRIORITY);
    if (selectSmsBowerCountryMode) {
...
function updateSmsBowerCountryModeUI() {
  ...
  if (rowSmsBowerCountryOrder) rowSmsBowerCountryOrder.style.display = fixedMode || randomMode || recommendedMode ? 'none' : '';
  if (rowSmsBowerCountryOrderActions) rowSmsBowerCountryOrderActions.style.display = fixedMode || randomMode || recommendedMode ? 'none' : '';
  if (rowSmsBowerFixedCountry) rowSmsBowerFixedCountry.style.display = fixedMode ? '' : 'none';
  [
    [btnSmsBowerCountryModeRandom, randomMode],
    [btnSmsBowerCountryModePriority, !randomMode && !fixedMode && !recommendedMode],
    [btnSmsBowerCountryModeRecommended, recommendedMode],
  ...
  if (btnSmsBowerProviderMenu) {
    btnSmsBowerProviderMenu.disabled = recommendedMode;
    btnSmsBowerProviderMenu.title = recommendedMode
      ? '推荐模式固定使用推荐线路池'
      : '';
  }
  const providerMenuModeKey = fixedMode
    ? `fixed:${fixedCountryId}`
    : (recommendedMode ? 'recommended' : (randomMode ? 'random' : 'priority'));
...
async function refreshSmsBowerCountryCatalog() {
  ...
  const catalogItems = (Array.isArray(catalog?.countries) ? catalog.countries : [])
    .map((entry) => {
      ...
      return {
        ...
        countryCode: String(entry?.countryCode || SMSBOWER_COUNTRY_CODE_BY_ID[id] || '').trim().toUpperCase(),
  ...
  const selectedIds = getSmsBowerFixedCountryId()
    ? [getSmsBowerFixedCountryId()]
    : smsbowerCountryOrderSelection;
  const autoProviderIds = getSmsBowerProviderIdsForCountryOrder(selectedIds, { cheapestOnly: true });
...
async function previewPhoneSmsBalance() {
  ...
  const provider = getSelectedPhoneSmsProvider();
  const smsBowerProvider = typeof PHONE_SMS_PROVIDER_SMSBOWER !== 'undefined' ? PHONE_SMS_PROVIDER_SMSBOWER : 'smsbower';
  const apiKey = provider === smsBowerProvider
  ...
  try {
    const url = provider === PHONE_SMS_PROVIDER_FIVE_SIM
      ...
      : new URL(provider === smsBowerProvider
    ...
    if (provider === PHONE_SMS_PROVIDER_FIVE_SIM) {
      ...
      displayPhoneSmsBalance.textContent = `${provider === smsBowerProvider ? 'SMSBower' : 'HeroSMS'} 余额 ${text || '未知'}`;
    }
  } catch (error) {
...
function updatePhoneVerificationSettingsUI() {
  ...
  const selectedProvider = typeof getSelectedPhoneSmsProvider === 'function'
    ? getSelectedPhoneSmsProvider()
    : normalizePhoneSmsProviderValue(selectPhoneSmsProvider?.value || latestState?.phoneSmsProvider || heroProviderValue);
  const provider = selectedProvider || providerOrderForDisplay[0] || heroProviderValue;
...
function applySettingsState(state) {
  ...
  setMail2925Mode(state?.mail2925Mode);
  {
    ...
    } else if (restoredEmailGenerator === smsbowerMailProvider) {
      selectEmailGenerator.value = smsbowerMailProvider;
    } else {
  ...
  if (typeof applySmsBowerMailSettingsState === 'function') {
    applySmsBowerMailSettingsState(state);
  }
  ...
  if (typeof smsbowerProviderIdsManualSelection !== 'undefined'
    && (typeof smsbowerProviderIdsManualOverride === 'undefined' || !smsbowerProviderIdsManualOverride)) {
    smsbowerProviderIdsManualSelection = Boolean(state?.smsbowerProviderIdsManual);
  }
  const hasManualSmsBowerProviderIds = typeof preserveSmsBowerProviderIdsManualValue === 'function'
    ? preserveSmsBowerProviderIdsManualValue()
    : false;
  ...
  if (typeof inputSmsBowerProviderIds !== 'undefined' && inputSmsBowerProviderIds) {
    ...
    const defaultSmsBowerProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
      ? DEFAULT_SMSBOWER_PROVIDER_IDS
      : '3193';
    const restoredSmsBowerCountryOrder = normalizeSmsBowerCountryOrderValue(
      (Array.isArray(state?.smsbowerCountryOrder) && state.smsbowerCountryOrder.length)
        ? state.smsbowerCountryOrder
        : DEFAULT_SMSBOWER_COUNTRY_ORDER
    );
    const autoSmsBowerProviderIds = getSmsBowerProviderIdsForCountryOrder(restoredSmsBowerCountryOrder);
    if (!hasManualSmsBowerProviderIds && (
      restoredSmsBowerProviderIds
      && restoredSmsBowerProviderIds !== defaultSmsBowerProviderIds
      && restoredSmsBowerProviderIds !== autoSmsBowerProviderIds
    )) {
      inputSmsBowerProviderIds.value = restoredSmsBowerProviderIds;
      smsbowerProviderIdsAutoValue = restoredSmsBowerProviderIds;
    }
  }
  if (typeof renderSmsBowerProviderMenu === 'function') renderSmsBowerProviderMenu();
  if (typeof inputSmsBowerMinPrice !== 'undefined' && inputSmsBowerMinPrice) {
    inputSmsBowerMinPrice.value = normalizeSmsBowerPriceValue(state?.smsbowerMinPrice || '');
  }
  if (typeof inputSmsBowerMaxPrice !== 'undefined' && inputSmsBowerMaxPrice) {
    inputSmsBowerMaxPrice.value = normalizeSmsBowerPriceValue(state?.smsbowerMaxPrice || DEFAULT_SMSBOWER_MAX_PRICE) || DEFAULT_SMSBOWER_MAX_PRICE;
  }
  if (typeof inputHeroSmsReuseEnabled !== 'undefined' && inputHeroSmsReuseEnabled) {
...
function isSmsBowerMailProvider(provider = selectMailProvider.value) {
  const smsbowerMailProvider = typeof SMSBOWER_MAIL_PROVIDER === 'string'
    ? SMSBOWER_MAIL_PROVIDER
    : 'smsbower-mail';
  return String(provider || '').trim().toLowerCase() === smsbowerMailProvider;
}
...
function normalizeSmsBowerMailBaseUrl(value = '') {
  if (window.SmsBowerMailUtils?.normalizeSmsBowerMailBaseUrl) {
    return window.SmsBowerMailUtils.normalizeSmsBowerMailBaseUrl(value);
  }
  const trimmed = String(value || '').trim();
  if (!trimmed) return DEFAULT_SMSBOWER_MAIL_BASE_URL;
  const candidate = /^[a-zA-Z][a-zA-Z\d+\-.]*:\/\//.test(trimmed) ? trimmed : `https://${trimmed}`;
  try {
    const parsed = new URL(candidate);
    if (!['http:', 'https:'].includes(parsed.protocol)) return DEFAULT_SMSBOWER_MAIL_BASE_URL;
    parsed.hash = '';
    parsed.search = '';
...
function normalizeSmsBowerMailServiceCode(value = '') {
  if (window.SmsBowerMailUtils?.normalizeSmsBowerMailServiceCode) {
    return window.SmsBowerMailUtils.normalizeSmsBowerMailServiceCode(value);
  }
  return String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '') || DEFAULT_SMSBOWER_MAIL_SERVICE_CODE;
}

function normalizeSmsBowerMailDomain(value = '') {
  if (window.SmsBowerMailUtils?.normalizeSmsBowerMailDomain) {
    return window.SmsBowerMailUtils.normalizeSmsBowerMailDomain(value);
  }
  const normalized = String(value || '').trim().toLowerCase();
...
function normalizeSmsBowerMailMaxPrice(value = '') {
  if (window.SmsBowerMailUtils?.normalizeSmsBowerMailMaxPrice) {
    return window.SmsBowerMailUtils.normalizeSmsBowerMailMaxPrice(value) || DEFAULT_SMSBOWER_MAIL_MAX_PRICE;
  }
  const numeric = Number(String(value ?? '').trim());
  if (!Number.isFinite(numeric) || numeric <= 0) return DEFAULT_SMSBOWER_MAIL_MAX_PRICE;
  return String(Math.round(numeric * 10000) / 10000);
}
...
function getSelectedEmailGenerator() {
  ...
  const smsbowerMailProvider = typeof SMSBOWER_MAIL_PROVIDER === 'string'
    ? SMSBOWER_MAIL_PROVIDER
    : 'smsbower-mail';
  if (generator === smsbowerMailProvider) return smsbowerMailProvider;
  return 'duck';
}
...
function getEmailGeneratorUiCopy() {
  ...
  const smsbowerMailProvider = typeof SMSBOWER_MAIL_PROVIDER === 'string'
    ? SMSBOWER_MAIL_PROVIDER
    : 'smsbower-mail';
  if (getSelectedEmailGenerator() === smsbowerMailProvider) {
    return {
      buttonLabel: '获取',
      placeholder: '点击获取 SMSBower TempMail 邮箱，或手动粘贴邮箱',
      successVerb: '获取',
...
function updateMailProviderUI() {
  ...
  const smsbowerMailProvider = typeof SMSBOWER_MAIL_PROVIDER === 'string'
    ? SMSBOWER_MAIL_PROVIDER
    : 'smsbower-mail';
  const useSmsBowerMailGenerator = selectedGenerator === smsbowerMailProvider;
  ...
  const showSmsBowerMailSettings = useSmsBowerMail || (useEmailGenerator && useSmsBowerMailGenerator);
  ...
  selectEmailGenerator.disabled = useHotmail || useLuckmail || useYydsMail || useSmsBowerMail || useCustomEmail || (useGeneratedAlias && !useGmail);
  if (useGmail) {
  ...
  try {
    const response = await chrome.runtime.sendMessage({
      ...
      payload: {
        ...
        mailProvider: selectMailProvider.value,
        mail2925Mode: getSelectedMail2925Mode(),
        smsbowerMailApiKey: inputSmsBowerMailApiKey ? inputSmsBowerMailApiKey.value.trim() : '',
        smsbowerMailBaseUrl: inputSmsBowerMailBaseUrl ? normalizeSmsBowerMailBaseUrl(inputSmsBowerMailBaseUrl.value) : '',
...
async function startAutoRunFromCurrentSettings() {
  ...
  const emailPoolSize = typeof getLockedRunCountFromEmailPool === 'function'    ? getLockedRunCountFromEmailPool()
    : 0;
...
function buildPhoneSmsProviderStatePatch(provider = getSelectedPhoneSmsProvider()) {
  ...
  if (normalizedProvider === smsBowerProvider) {
    ...
    return {
      ...
      smsbowerProviderIds: normalizeSmsBowerProviderIdsValue(inputSmsBowerProviderIds?.value || latestState?.smsbowerProviderIds || defaultSmsBowerProviderIds),
...
btnSmsBowerApiDocs?.addEventListener('click', (event) => {
  event.preventDefault();
  openExternalUrl('https://smsbower.app/cn/api?page=client');
});
btnSmsBowerFixedCountryMenu?.addEventListener('click', (event) => {
  event.preventDefault();
  renderSmsBowerFixedCountryMenu();
  setSmsBowerFixedCountryMenuOpen(btnSmsBowerFixedCountryMenu.getAttribute('aria-expanded') !== 'true');
});
btnSmsBowerProviderMenu?.addEventListener('click', (event) => {
  event.preventDefault();
  renderSmsBowerProviderMenu();
  setSmsBowerProviderMenuOpen(btnSmsBowerProviderMenu.getAttribute('aria-expanded') !== 'true');
});
...
btnSmsBowerCountryModeRecommended?.addEventListener('click', (event) => {
  event.preventDefault();
  setSmsBowerCountryMode(SMSBOWER_COUNTRY_MODE_RECOMMENDED, { persist: true });
  updateHeroSmsPlatformDisplay();
});
...
inputSmsBowerProviderIds?.addEventListener('blur', () => {
  inputSmsBowerProviderIds.value = normalizeSmsBowerProviderIdsValue(inputSmsBowerProviderIds.value);
  updateSmsBowerProviderMenuSummary();
  saveSettings({ silent: true }).catch(() => { });
});

[inputSmsBowerMinPrice, inputSmsBowerMaxPrice].forEach((input) => {
  input?.addEventListener('input', () => {
    markSettingsDirty(true);
    scheduleSettingsAutoSave();
  });
  input?.addEventListener('blur', () => {
    input.value = normalizeSmsBowerPriceValue(input.value) || (input === inputSmsBowerMaxPrice ? DEFAULT_SMSBOWER_MAX_PRICE : '');
    saveSettings({ silent: true }).catch(() => { });
  });
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  switch (message.type) {
    ...
    case 'DATA_UPDATED': {
      ...
      if (message.payload.smsbowerMailApiKey !== undefined && inputSmsBowerMailApiKey) {
        inputSmsBowerMailApiKey.value = message.payload.smsbowerMailApiKey || '';
      }
      if (message.payload.smsbowerMailBaseUrl !== undefined && inputSmsBowerMailBaseUrl) {
        inputSmsBowerMailBaseUrl.value = normalizeSmsBowerMailBaseUrl(message.payload.smsbowerMailBaseUrl);
      }
      if (message.payload.smsbowerMailServiceCode !== undefined && inputSmsBowerMailServiceCode) {
        inputSmsBowerMailServiceCode.value = normalizeSmsBowerMailServiceCode(message.payload.smsbowerMailServiceCode);
      }
      if (message.payload.smsbowerMailDomain !== undefined && inputSmsBowerMailDomain) {
        inputSmsBowerMailDomain.value = normalizeSmsBowerMailDomain(message.payload.smsbowerMailDomain);
      }
      ...
      if (message.payload.smsbowerProviderIdsManual !== undefined && !smsbowerProviderIdsManualOverride) {
        smsbowerProviderIdsManualSelection = Boolean(message.payload.smsbowerProviderIdsManual);
      }
      if (message.payload.smsbowerProviderIds !== undefined && inputSmsBowerProviderIds) {
        if (typeof preserveSmsBowerProviderIdsManualValue !== 'function'
          || !preserveSmsBowerProviderIdsManualValue()) {
          const incomingSmsBowerProviderIds = normalizeSmsBowerProviderIdsValue(message.payload.smsbowerProviderIds || '');
          const defaultSmsBowerProviderIds = typeof DEFAULT_SMSBOWER_PROVIDER_IDS !== 'undefined'
            ? DEFAULT_SMSBOWER_PROVIDER_IDS
            : '3193';
          const currentSmsBowerCountryOrder = normalizeSmsBowerCountryOrderValue(
            (Array.isArray(message.payload.smsbowerCountryOrder) && message.payload.smsbowerCountryOrder.length)
              ? message.payload.smsbowerCountryOrder
              : ((Array.isArray(latestState?.smsbowerCountryOrder) && latestState.smsbowerCountryOrder.length) ? latestState.smsbowerCountryOrder : DEFAULT_SMSBOWER_COUNTRY_ORDER)
          );
          const autoSmsBowerProviderIds = getSmsBowerProviderIdsForCountryOrder(currentSmsBowerCountryOrder);
          if (
            incomingSmsBowerProviderIds
            && incomingSmsBowerProviderIds !== defaultSmsBowerProviderIds
            && incomingSmsBowerProviderIds !== autoSmsBowerProviderIds
          ) {
            inputSmsBowerProviderIds.value = incomingSmsBowerProviderIds;
            smsbowerProviderIdsAutoValue = incomingSmsBowerProviderIds;
          }
        }
      }
      if (message.payload.smsbowerMinPrice !== undefined && inputSmsBowerMinPrice) {
        inputSmsBowerMinPrice.value = normalizeSmsBowerPriceValue(message.payload.smsbowerMinPrice);
      }
      if (message.payload.smsbowerMaxPrice !== undefined && inputSmsBowerMaxPrice) {
        inputSmsBowerMaxPrice.value = normalizeSmsBowerPriceValue(message.payload.smsbowerMaxPrice || DEFAULT_SMSBOWER_MAX_PRICE) || DEFAULT_SMSBOWER_MAX_PRICE;
      }
      if (
...
document.addEventListener('click', (event) => {
  ...
  const clickedInsideSmsBowerFixedCountryMenu = Boolean(smsbowerFixedCountryMenuShell?.contains(event.target));
  const clickedInsideSmsBowerProviderMenu = Boolean(smsbowerProviderMenuShell?.contains(event.target));
  ...
  if (btnSmsBowerFixedCountryMenu?.getAttribute('aria-expanded') === 'true' && !clickedInsideSmsBowerFixedCountryMenu) {
    setSmsBowerFixedCountryMenuOpen(false);
  }
  if (btnSmsBowerProviderMenu?.getAttribute('aria-expanded') === 'true' && !clickedInsideSmsBowerProviderMenu) {
    setSmsBowerProviderMenuOpen(false);
  }
